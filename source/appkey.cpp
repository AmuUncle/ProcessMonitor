...
void AppKey::start()
{
    ...
    if (!keyFile.exists() || keyFile.size() == 0) {
        emit SignalIsOver();
        GfxMessageBox("密钥文件丢失,请联系供应商!", "错误");
        exit(0);
    }
    ...
    keyData = getXorEncryptDecrypt(keyData, 101, 110);
    QStringList data = keyData.split("|");

    if (data.count() != 6) {
        emit SignalIsOver();
        GfxMessageBox("注册码文件已损坏,程序将自动关闭!", "错误");
        exit(0);
    }
    ...
    if (keyUseDate) {
        ...
        if (nowDate > keyDate) {
            emit SignalIsOver();
            GfxMessageBox("软件已到期,请联系供应商更新注册码!", "错误");
            exit(0);
        }
    }
...
void AppKey::checkTime()
{
    ...
    if (startTime.secsTo(now) >= (keyRun * 60)) {
        emit SignalIsOver();
        GfxMessageBox("试运行时间已到,请联系供应商更新注册码!", "错误");
        exit(0);
    }
}

QString AppKey::getXorEncryptDecrypt(const QString &data, char key, char key2)
{
    //采用异或加密,也可以自行更改算法
...
bool AppKey::checkCount(int count)
{
    if (keyUseCount) {
        if (count >= keyCount) {
            emit SignalIsOver();
            GfxMessageBox("设备数量超过限制,请联系供应商更新注册码!", "错误");
            return false;
        }
    }
